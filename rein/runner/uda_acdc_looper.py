...
class UDAACDCIterBasedTrainLoop(IterBasedTrainLoop):
    ...
    def run_iter(self, data_batch, is_src=False, is_trg=False) -> None:
        ...
        for i in range(self.batch_size):
            trg_data_batch['data_samples'][i].gt_sem_seg.data = PixelData(**dict(gt_sem_seg=mixed_lbl[i].squeeze())).gt_sem_seg.data
        ...
        self.runner.call_hook(
            ...
            outputs={**src_outputs, **trg_outputs}  # , **masked_outputs   **src_outputs,   **sty_outputs,
        )
...
@LOOPS.register_module()
class UDAACDCTestLoop(TestLoop):
    """Loop for test.
