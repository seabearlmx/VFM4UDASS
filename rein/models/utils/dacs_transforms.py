...
def denorm_(img, mean, std):
    # print('img.mul_(std) dtype:', img.mul_(std).dtype)
    # print('img.mul_(std).add_(mean) dtype:', img.mul_(std).add_(mean).dtype)
    # print('img.mul_(std).add_(mean).div_(255.0) dtype:', img.mul_(std).add_(mean).div_(255.0).dtype)
...
def get_class_masks(labels):
    class_masks = []
    for label in labels:
        # print(torch.from_numpy(np.array(label)).cuda())
        # label = torch.stack(label).cuda()
